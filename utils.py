...
def init_population_with_fit(population_size, net, fitness, sigma, max_path_length, theta_0, env, deterministic=False):
    """
...
def train_PG(policy, policy_optimizer,
    ...
    for param in state_dict:
        # print(sigma)
        # print(list(policy.parameters())[i].grad.data.numpy())
        gradients[param] = list(policy.parameters())[i].grad
        # gradients[param] = torch.zeros(list(policy.parameters())[i].shape) + torch.Tensor([sigma])
        i += 1

    # print("After PG of iter %d:" % itr, list(policy.parameters()))

    new_objective = evaluate_obj(policy, policy.state_dict(), max_path_length, env)

    # return new_objective, gradients
    return ave_return, gradients
...
def policy_gradient(net, lr, env, max_path_length, num_episode=1):
    ...
    for i in range(num_episode):
        ...
        # print(len(rtg_all_t), rtg_all_t, sep="\n")
        # print(len(log_prob_all_t), log_prob_all_t, sep="\n")

        log_prob_all_t = torch.stack(log_prob_all_t, dim=1).squeeze()

        # print(log_prob_all_t)
